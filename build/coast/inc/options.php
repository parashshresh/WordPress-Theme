...
function coast_pro_page_choices() {
    $pages = get_pages();
    $choices = array();
    $choices[0] = esc_html__( '--Select--', 'coast' );
...
if ( ! function_exists( 'coast_pro_selected_sidebar' ) ) :
    ...
    function coast_pro_selected_sidebar() {
        $coast_pro_selected_sidebar = array(
            'sidebar-1'             => esc_html__( 'Default Sidebar', 'coast' ),
            'optional-sidebar'      => esc_html__( 'Optional Sidebar 1', 'coast' ),
            'optional-sidebar-2'    => esc_html__( 'Optional Sidebar 2', 'coast' ),
            'optional-sidebar-3'    => esc_html__( 'Optional Sidebar 3', 'coast' ),
            'optional-sidebar-4'    => esc_html__( 'Optional Sidebar 4', 'coast' ),
        );
...
if ( ! function_exists( 'coast_pro_pagination_options' ) ) :
    ...
    function coast_pro_pagination_options() {
        $coast_pro_pagination_options = array(
            'numeric'   => esc_html__( 'Numeric', 'coast' ),
            'default'   => esc_html__( 'Default(Older/Newer)', 'coast' ),
        );
...
if ( ! function_exists( 'coast_pro_get_spinner_list' ) ) :
    ...
    function coast_pro_get_spinner_list() {
        $arr = array(
            'default'               => esc_html__( 'Default', 'coast' ),
            'spinner-wheel'         => esc_html__( 'Wheel', 'coast' ),
            'spinner-double-circle' => esc_html__( 'Double Circle', 'coast' ),
            'spinner-two-way'       => esc_html__( 'Two Way', 'coast' ),
            'spinner-umbrella'      => esc_html__( 'Umbrella', 'coast' ),
            'spinner-dots'          => esc_html__( 'Dots', 'coast' ),
            'spinner-one-way'       => esc_html__( 'One Way', 'coast' ),
            'spinner-fidget'        => esc_html__( 'Fidget', 'coast' ),
        );
...
if ( ! function_exists( 'coast_pro_switch_options' ) ) :
    ...
    function coast_pro_switch_options() {
        $arr = array(
            'on'        => esc_html__( 'Enable', 'coast' ),
            'off'       => esc_html__( 'Disable', 'coast' )
        );
...
if ( ! function_exists( 'coast_pro_hide_options' ) ) :
    ...
    function coast_pro_hide_options() {
        $arr = array(
            'on'        => esc_html__( 'Yes', 'coast' ),
            'off'       => esc_html__( 'No', 'coast' )
        );
...
if ( ! function_exists( 'coast_pro_sortable_sections' ) ) :
    ...
    function coast_pro_sortable_sections() {
        $sections = array(
            'hero_banner'                       => esc_html__( 'Hero Banner', 'coast' ),
            'featured'                          => esc_html__( 'Featured', 'coast' ),
            'blog'                              => esc_html__( 'Blog', 'coast' ),
            'testimonial'                       => esc_html__( 'Testimonial', 'coast' ),
            'destination'                       => esc_html__( 'Destination', 'coast' ),
            'about'                             => esc_html__( 'About', 'coast' ),
            'gallery'                           => esc_html__( 'Gallery', 'coast' ),
            'counter'                           => esc_html__( 'Counter', 'coast' ),
            'subscribe'                         => esc_html__( 'Subscribe Us', 'coast' ),
        );
...
if ( ! function_exists( 'coast_pro_blog_content_type' ) ) :
    ...
    function coast_pro_blog_content_type() {
        $coast_pro_blog_content_type = array(
            'category'  => esc_html__( 'Category', 'coast' ),
        );
