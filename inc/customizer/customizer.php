...
function coast_pro_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'coast_pro_theme_options[header_title_color]', array(
		'priority'			=> 5,
		'label'             => esc_html__( 'Header Title Color', 'coast-pro' ),
		'section'           => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'coast_pro_theme_options[header_tagline_color]', array(
		'priority'			=> 6,
		'label'             => esc_html__( 'Header Tagline Color', 'coast-pro' ),
		'section'           => 'colors',
	) ) );
	...
	$wp_customize->add_control( 'coast_pro_theme_options[header_txt_logo_extra]', array(
		'priority'			=> 50,
		'type'				=> 'radio',
		'label'             => esc_html__( 'Site Identity Extra Options', 'coast-pro' ),
		'section'           => 'title_tagline',
		'choices'				=> array( 
			'hide-all'     => esc_html__( 'Hide All', 'coast-pro' ),
			'show-all'     => esc_html__( 'Show All', 'coast-pro' ),
			'title-only'   => esc_html__( 'Title Only', 'coast-pro' ),
			'tagline-only' => esc_html__( 'Tagline Only', 'coast-pro' ),
			'logo-title'   => esc_html__( 'Logo + Title', 'coast-pro' ),
			'logo-tagline' => esc_html__( 'Logo + Tagline', 'coast-pro' ),
			)
	) );
	...
	$wp_customize->add_panel( 'coast_pro_theme_options_panel' , array(
	    'title'      => esc_html__( 'Theme Options','coast-pro' ),
	    'description'=> esc_html__( 'Coast Pro Theme Options.', 'coast-pro' ),
	    'priority'   => 150,
	) );
	...
	$wp_customize->add_panel( 'coast_pro_front_page_panel' , array(
	    'title'      => esc_html__( 'Front Page','coast-pro' ),
	    'description'=> esc_html__( 'Front Page Theme Options.', 'coast-pro' ),
	    'priority'   => 140,
	) );
...
function coast_pro_customize_control_js() {
	...
	$coast_pro_reset_data = array(
		'reset_message' => esc_html__( 'Refresh the customizer page after saving to view reset effects', 'coast-pro' )
	);
