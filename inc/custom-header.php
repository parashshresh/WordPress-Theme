 ...
 *
 * @uses coast_pro_header_style()
 */
function coast_pro_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'coast_pro_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/header-image.jpg',
		'default-text-color'     => '181818',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'coast_pro_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'coast-pro' ),
		),
	) );
}
add_action( 'after_setup_theme', 'coast_pro_custom_header_setup' );

if ( ! function_exists( 'coast_pro_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see coast_pro_custom_header_setup().
	 */
	function coast_pro_header_style() {
		...
		wp_add_inline_style( 'coast-pro-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'coast_pro_header_style', 10 );