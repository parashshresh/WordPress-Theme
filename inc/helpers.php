...
if( ! function_exists( 'coast_pro_check_enable_status' ) ):
	/**
	 * Check status of content.
	 *
	 * @since Coast Pro 1.0.0
	 */
  	function coast_pro_check_enable_status( $input, $content_enable ){
...
add_filter( 'coast_pro_section_status', 'coast_pro_check_enable_status', 10, 2 );


if ( ! function_exists( 'coast_pro_is_sidebar_enable' ) ) :
	/**
	 * Check if sidebar is enabled in meta box first then in customizer
	 *
	 * @since Coast Pro 1.0.0
	 */
	function coast_pro_is_sidebar_enable() {
		$options               = coast_pro_get_theme_options();
		...
		if ( is_home() ) {
			...
			if ( ! empty( $post_id ) )
				$post_sidebar_position = get_post_meta( $post_id, 'coast-pro-sidebar-position', true );
			else
				$post_sidebar_position = '';
		} elseif ( is_archive() || is_search() ) {
			$post_sidebar_position = '';
		} else {
			$post_sidebar_position = get_post_meta( get_the_id(), 'coast-pro-sidebar-position', true );
			if ( is_single() ) {
...
if ( ! function_exists( 'coast_pro_is_frontpage_content_enable' ) ) :
	/**
	 ...
	 */
	function coast_pro_is_frontpage_content_enable( $status ) {
		if ( is_front_page() ) {
			$options = coast_pro_get_theme_options();
...
add_filter( 'coast_pro_filter_frontpage_content_enable', 'coast_pro_is_frontpage_content_enable' );

add_action( 'coast_pro_action_pagination', 'coast_pro_pagination', 10 );
if ( ! function_exists( 'coast_pro_pagination' ) ) :
		...
		if ( true == $options['pagination_enable'] ) {
			...
			elseif ( in_array( $pagination, array( 'infinite', 'numeric' ) ) ) :
				the_posts_pagination( array(
				    'mid_size' => 4,
				    'prev_text' => coast_pro_get_svg( array( 'icon' => 'up' ) ),
				    'next_text' => coast_pro_get_svg( array( 'icon' => 'up' ) ) ,
				) );
			endif;
		}
	}
...
add_action( 'coast_pro_action_post_pagination', 'coast_pro_post_pagination', 10 );
if ( ! function_exists( 'coast_pro_post_pagination' ) ) :
	 ...
	 * @since Coast Pro 1.0.0
	 */
	function coast_pro_post_pagination() {
		the_post_navigation( array(
			'prev_text'	=> coast_pro_get_svg( array( 'icon' => 'up' ) ) .  '<span>%title</span>',
            'next_text' => '<span>%title</span>' . coast_pro_get_svg( array( 'icon' => 'up' ) ),
		) );
	}
endif;


if ( ! function_exists( 'coast_pro_excerpt_length' ) ) :
	/**
	 * long excerpt
	 * 
	 * @since Coast Pro 1.0.0
	 ...
	 */
	function coast_pro_excerpt_length( $length ){
		if ( is_admin() ) {
			return $length;
		}
...
add_filter( 'excerpt_length', 'coast_pro_excerpt_length', 999 );


if ( ! function_exists( 'coast_pro_excerpt_more' ) ) :
	// Read more
	function coast_pro_excerpt_more( $more ){
		if ( is_admin() ) {
			return $more;
		}
...
add_filter( 'excerpt_more', 'coast_pro_excerpt_more' );


if ( ! function_exists( 'coast_pro_trim_content' ) ) :
	/**
	 ...
	 */
	function coast_pro_trim_content( $length = 40, $post_obj = null ) {
		global $post;
		if ( is_null( $post_obj ) ) {
			$post_obj = $post;
		}
	   ...
	   return apply_filters( 'coast_pro_trim_content', $trimmed_content );
	}
endif;
...
function coast_pro_font_choices() {
    ...
    return apply_filters( 'coast_pro_font_choices', $font_family_arr );
}

if ( ! function_exists( 'coast_pro_layout' ) ) :
	/**
	 * Check home page layout option
	 *
	 * @since Coast Pro 1.0.0
	 *
	 * @return string Coast Pro layout value
	 */
	function coast_pro_layout() {
		$options = coast_pro_get_theme_options();

		$sidebar_position = $options['sidebar_position'];
		$sidebar_position_post = $options['post_sidebar_position'];
		$sidebar_position_page = $options['page_sidebar_position'];
		$sidebar_position = apply_filters( 'coast_pro_sidebar_position', $sidebar_position );
		// Check if single and static blog page
		if ( is_singular() || is_home() ) {
			if ( is_home() ) {
				$post_sidebar_position = get_post_meta( get_option( 'page_for_posts' ), 'coast-pro-sidebar-position', true );
			} else {
				$post_sidebar_position = get_post_meta( get_the_ID(), 'coast-pro-sidebar-position', true );
			}
 ...
 */
function coast_pro_include_svg_icons() {
...
add_action( 'wp_footer', 'coast_pro_include_svg_icons', 9999 );
...
function coast_pro_get_svg( $args = array() ) {
	 ...
	 *
	 * Example 1 with title: <?php echo coast_pro_get_svg( array( 'icon' => 'arrow-right', 'title' => __( 'This is the title', 'textdomain' ) ) ); ?>
	 *
	 * Example 2 with title and description: <?php echo coast_pro_get_svg( array( 'icon' => 'arrow-right', 'title' => __( 'This is the title', 'textdomain' ), 'desc' => __( 'This is the description', 'textdomain' ) ) ); ?>
	 *
 ...
 */
function coast_pro_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
	if ( 'primary' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ( 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title = $title . coast_pro_get_svg( array( 'icon' => 'down' ) );
			}
		}
	}
...
add_filter( 'nav_menu_item_title', 'coast_pro_dropdown_icon_to_menu_link', 10, 4 );
 ...
 */
function coast_pro_social_links_icons() {
	...
	/**
	 * Filter Coast Pro social links icons.
	 *
	 * @since Coast Pro 1.0.0
	 *
	 * @param array $social_links_icons Array of social links icons.
	 */
	return apply_filters( 'coast_pro_social_links_icons', $social_links_icons );
}
 ...
 */
function coast_pro_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Get supported social icons.
	$social_icons = coast_pro_social_links_icons();

	// Change SVG icon inside social links menu if there is supported URL.
	if ( 'social' === $args->theme_location ) {
		foreach ( $social_icons as $attr => $value ) {
			if ( false !== strpos( $item_output, $attr ) ) {
				$item_output = str_replace( $args->link_after, '</span>' . coast_pro_get_svg( array( 'icon' => esc_attr( $value ) ) ), $item_output );
			}
		}
	}
...
add_filter( 'walker_nav_menu_start_el', 'coast_pro_nav_menu_social_icons', 10, 4 );
 ...
 */
function coast_pro_return_social_icon( $social_link ) {
	// Get supported social icons.
	$social_icons = coast_pro_social_links_icons();
	...
	foreach ( $social_icons as $attr => $value ) {
		if ( false !== strpos( $social_link, $attr ) ) {
			return coast_pro_get_svg( array( 'icon' => esc_attr( $value ) ) );
		}
	}
}
 ...
 */
function coast_pro_menu_fallback_cb( $args ){
 ...
 */
function coast_pro_is_frontpage() {
 ...
 */
function coast_pro_is_latest_posts() {
	return ( is_front_page() && is_home() );
}
 ...
 */
function coast_pro_is_blog_page() {
	return ( ! is_front_page() && is_home() );
}

if ( ! function_exists( 'coast_pro_simple_breadcrumb' ) ) :
	/**
	 * Simple breadcrumb.
	 *
	 * @param  array $args Arguments
	 */
	function coast_pro_simple_breadcrumb( $args = array() ) {
		/**
		 * Add breadcrumb.
		 *
		 */
...
add_action( 'coast_pro_simple_breadcrumb', 'coast_pro_simple_breadcrumb' , 10 );
...
function coast_pro_get_homepage_sections(){
	$options = coast_pro_get_theme_options();
